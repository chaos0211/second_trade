...
def fetch_html(url: str, referer: str | None = None) -> etree._Element | None:
    ...
    for attempt in range(1, 7):
        ...
        if resp.status_code == 503:
            # If server returns openService/503 to bots, try once without Cookie.
            if (not cookie_dropped) and ("Cookie" in headers):
                headers.pop("Cookie", None)
                cookie_dropped = True

            # Backoff a bit more and add jitter to look less like a bot.
            time.sleep(random.uniform(3.0, 5.0) * attempt)
        else:
...
def prewarm_for(url: str) -> None:
    """Warm up the session with a few lightweight GETs to reduce 503 on list pages."""
    try:
        # 1) site home
        SESSION.get(BASE, headers={"User-Agent": HEADERS.get("User-Agent", "")}, timeout=15)
        time.sleep(random.uniform(0.6, 1.2))

        # 2) section home (first path segment)
        m = re.search(r"https?://[^/]+/([^/]+)/", url)
        seg = m.group(1) if m else ""
        if seg:
            section_home = urljoin(BASE, f"/{seg}/")
            SESSION.get(section_home, headers={"User-Agent": HEADERS.get("User-Agent", ""), "Referer": BASE}, timeout=15)
            time.sleep(random.uniform(0.6, 1.2))

        # 3) the target url (no parsing, just to establish cookies)
        SESSION.get(url, headers={"User-Agent": HEADERS.get("User-Agent", ""), "Referer": urljoin(BASE, f"/{seg}/") if seg else BASE}, timeout=15)
        time.sleep(random.uniform(0.6, 1.2))
    except Exception:
        # prewarm is best-effort; ignore any errors
        return
...
def get_next_page_url(doc: etree._Element, current_url: str) -> str | None:
    ...
    - then a.next
    """
    next_hrefs = doc.xpath('//div[@class="page-box"]/div[@class="pagebar"]/a[@class="next" and normalize-space(string(.))="下一页"]/@href')
...
def discover_brands(start_doc: etree._Element, current_url: str, limit: int = 10) -> list[dict]:
    ...
    print("========== anchors(debug) ==========")
    for i, a in enumerate(anchors[:60], start=1):
    ...
    # Build full brand list first
    brands_all: list[dict] = []
    for a in anchors:
        ...
        # e.g. https://detail.zol.com.cn/cell_phone_index/... -> cell_phone_index
        m = re.search(r"https?://[^/]+/([^/]+)/", url)
        index_type = m.group(1) if m else ""

        brands_all.append({"brand": name, "url": url, "index_type": index_type})

    # Take first 11, then drop "不限" if present, then apply limit
    top11 = brands_all[:11]
    top11 = [b for b in top11 if b.get("brand") != "不限"]
    brands = top11[:limit]
...
def crawl_all_pages(list_url: str, brand: str, index_type: str, max_pages: int = 200, sleep_sec: float = 0.6) -> list[dict]:
    ...
    for page_idx in range(1, max_pages + 1):
        ...
        # For the first page, using the section home as referer works better than bare BASE.
        section_home = urljoin(BASE, f"/{index_type}/") if index_type else BASE
        doc = fetch_html(url, referer=prev_url or section_home)
        if doc is None:
...
if __name__ == "__main__":
    ...
    START_URL = "https://detail.zol.com.cn/digital_tv/"
    ...
    # Output file name follows the top path segment in START_URL, e.g. /zsyxj/ -> zol_products/zsyxj.csv
    m = re.search(r"https?://[^/]+/([^/]+)/", START_URL)
    start_slug = m.group(1) if m else "products"
    out = f"zol_products/{start_slug}.csv"
