...
const team = ref([]);
const selectedIndex = ref(2); // 默认选中第三个，模仿图片
...
const generatePokemon = (index) => {
  ...
  return {
    ...
    pokedexId: base.id,
    name: base.name, // 实际应用中可能是昵称
    speciesName: base.species,
    level: level,
    gender: Math.random() > 0.5 ? 'female' : 'male', // 简化性别逻辑
    maxHp,
    currentHp,
    isShiny: Math.random() > 0.95, // 5%概率闪光
  };
};
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-cyan-100 via-purple-100 to-blue-100 p-4 font-sans select-none flex flex-col items-center">
    ...
    <!-- 卡片网格容器 -->
    <!-- 核心响应式逻辑:
         grid-cols-2 (手机) -> grid-cols-3 (平板) -> grid-cols-4 (小桌面) -> grid-cols-6 (大桌面)
         gap-4: 间距
    -->
    ...
    <div class="flex flex-col items-center gap-8 pb-12">
      ...
      <!-- Logo (使用 SVG 模拟 Pokemon Logo 风格) -->
